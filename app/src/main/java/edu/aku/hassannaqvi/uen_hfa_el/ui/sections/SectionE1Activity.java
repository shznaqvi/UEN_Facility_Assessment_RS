...
import edu.aku.hassannaqvi.uen_hfa_el.databinding.ActivitySectionE1Binding;
...
public class SectionE1Activity extends AppCompatActivity {
    ...
    private boolean insertNewRecord() {
        if (!moduleE.getUid().equals("") || MainApp.superuser) return true;
        moduleE.populateMeta();
        long rowId = 0;
        try {
            rowId = db.addModuleE(moduleE);
    ...
    private boolean updateDB() {
        ...
        long updcount = 0;
        try {
            updcount = db.updatesModuleEColumn(TableContracts.ModuleETable.COLUMN_SE1, moduleE.sE1toString());
        } catch (JSONException e) {
            e.printStackTrace();
